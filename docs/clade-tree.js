...
const state = {
  ...
  selectedNodes: new Set(),  // Changed to Set for multi-select
  showLabels: true,
  showLegend: false,
  showTable: false,
...
function setupEventListeners() {
  ...
  d3.select('#show-labels').on('change', function() {
    ...
    elements.g.selectAll('.node text').style('display', state.showLabels ? 'block' : 'none');
  });
  ...
  d3.select('#label-size').on('input', function() {
    ...
    elements.g.selectAll('.node text').style('font-size', state.settings.labelSize + 'px');
  });
...
function parseSubcladeYaml(yaml, filename) {
  ...
  function parseMutationLine(trimmed) {
    if (trimmed.match(/^-\s*locus:/)) {
      saveCurrentItem();
      const match = trimmed.match(/^-\s*locus:\s*["']?([^"'\s#]+)["']?/);
...
function buildHierarchy(nodes) {
  ...
  if (roots.length > 1) {
    return { id: '__root__', children: roots, isVirtualRoot: true };
  }
  
  if (roots.length === 0) {
    return { id: '__empty__', children: [] };
  }
...
function render() {
  ...
  const nodeHeight = Math.max(16, Math.min(30, (height - 80) / leafCount));
  
  // Use customizable edge length
  const treeLayout = d3.tree()
    .nodeSize([nodeHeight, state.settings.edgeLength])
  ...
  if (state.showLegend) updateLegend();
...
function centerView(root, width, height, hasVirtualRoot) {
  ...
  const translateX = (width - treeWidth * scale) / 2 - minY * scale + 60;
  const translateY = (height - treeHeight * scale) / 2 - minX * scale + 40;
